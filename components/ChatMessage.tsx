...
const PreComponent = ({ children, ...props }: any) => {
  ...
  return (
    <div className="relative group w-full overflow-hidden">
      <div className="absolute right-2 top-2 z-20 flex items-center space-x-2 opacity-0 group-hover:opacity-100 md:opacity-0 transition-opacity">
        <button
          onClick={handleCopy}
      ...
      <div className={`flex max-w-[95%] md:max-w-[85%] ${isUser ? 'flex-row-reverse' : 'flex-row'} items-start min-w-0`}>
        ...
        <div className={`flex flex-col group/msg min-w-0 ${isUser ? 'items-end' : 'items-start'} overflow-hidden`}>
          <div className={`px-3 md:px-4 py-2.5 md:py-3 rounded-2xl shadow-sm relative w-full overflow-hidden ${
              ...
              <div className="mb-2 md:mb-3">
                {isImage ? (
                  ...
                  <div className="flex items-center gap-2 p-2 bg-white/50 border border-gray-200 rounded-xl">
                    ...
                    <span className="text-[9px] font-bold text-gray-700 truncate">{message.attachment.name || 'Document.pdf'}</span>
                    <a href={message.attachment.url} target="_blank" rel="noreferrer" className="ml-auto text-indigo-600"><i className="fa-solid fa-arrow-up-right-from-square text-[9px]"></i></a>
                  </div>
                )}
              </div>
            )}
            ...
            {!isUser && message.text && (
              <div className="flex items-center gap-3 mt-3 pt-3 border-t border-gray-50 opacity-0 group-hover/msg:opacity-100 md:opacity-0 transition-opacity">
                <button 
                  onClick={handleCopyAll}
                  className="text-gray-400 hover:text-indigo-600 transition-colors flex items-center gap-1.5 text-[9px] font-bold"
                  title="Sao chép"
                >
